...
def main():
    ...
    # Calculate chunk size for MVPFormer (to avoid shared memory OOM)
    ...
    # Build MVPFormer with chunk_size 
    mvpformer = build_mvpformer_from_yaml(
        repo_path=config.mvpformer.repo_path,
        size_input=chunk_size  
    )
    
    # Load pretrained weights with partial loading (handles shape mismatch)
    ...
    print(f"MVPFormer loaded: {stats['loaded_keys']}/{stats['total_keys']} keys ({stats['loaded_keys']/stats['total_keys']*100:.1f}%)")
    
    n_params = sum(p.numel() for p in mvpformer.parameters())
    print(f"  Parameters: {n_params:,} ({n_params/1e6:.1f}M)")
    print(f"  All parameters frozen")
    print(f"  Input size: {chunk_size} samples per chunk")
